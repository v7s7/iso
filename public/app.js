...
function downloadXlsx(matrix,fileBase,sheetName='البيانات'){if(!matrix||matrix.length<2){alert('لا توجد بيانات ظاهرة للتصدير.');return}let bytes=buildXlsxBytes(matrix,sheetName),blob=new Blob([bytes],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`${fileBase}_${TODAY}.xlsx`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)}
let state={view:'dashboard',reqCode:null,adminTab:'users',flash:null,search:'',dashDepartment:'all',dashEmployee:'all',dashService:'all',dashStatus:'all',dashReqCode:'',dashDateFrom:'',dashDateTo:'',adminSearch:'',adminDept:'all',adminRole:'all',adminStatus:'all',adminForm:null,newDraft:{serviceId:'',requestDate:TODAY,subject:'',notes:''}};
...
function render(){let d=load(),u=user(),root=document.getElementById('app'),active=document.activeElement,focusId=active?.id,selection=typeof active?.selectionStart==='number'?[active.selectionStart,active.selectionEnd]:null;
 ...
 if(u.forcePasswordChange){root.innerHTML=forcePasswordView();bindForcePassword();return}let body=state.view==='dashboard'?dashboard(d,u):state.view==='new'?newView(d,u):state.view==='detail'?detailView(d,u,state.reqCode):state.view==='admin'?adminView(d,u):state.view==='password'?passwordView():dashboard(d,u);root.innerHTML=shell(body,d,u);enhanceRenderedUi(d);bindCommon(d,u);if(focusId){let next=document.getElementById(focusId);if(next){next.focus();if(selection&&next.setSelectionRange)next.setSelectionRange(...selection)}}state.flash=null}
function bindLogin(){
...
function applyRouteFromLocation(){let [view,reqCode]=location.hash.replace(/^#/,'').split('/');if(!['dashboard','new','detail','admin','password'].includes(view))view='dashboard';state.view=view;state.reqCode=view==='detail'&&reqCode?reqCode:null;history.replaceState({iso:true,view:state.view,reqCode:state.reqCode},'',`#${state.view}${state.reqCode?`/${state.reqCode}`:''}`)}
