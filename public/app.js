...
function downloadXlsx(matrix,fileBase,sheetName='البيانات'){if(!matrix||matrix.length<2){alert('لا توجد بيانات ظاهرة للتصدير.');return}let bytes=buildXlsxBytes(matrix,sheetName),blob=new Blob([bytes],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`${fileBase}_${TODAY}.xlsx`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)}
let state={view:'dashboard',reqCode:null,adminTab:'users',flash:null,search:'',dashDepartment:'all',dashEmployee:'all',dashService:'all',dashStatus:'all',dashReqCode:'',dashDateFrom:'',dashDateTo:'',adminSearch:'',adminDept:'all',adminRole:'all',adminStatus:'all',adminForm:null};
function resetDashboardFilters(){state.search='';state.dashDepartment='all';state.dashEmployee='all';state.dashService='all';state.dashStatus='all';state.dashReqCode='';state.dashDateFrom='';state.dashDateTo=''}
...
function filteredRequests(d,u){let rows=baseVisibleRequests(d,u).slice();
 ...
 if(state.dashStatus!=='all')rows=rows.filter(r=>matchesDashStatus(r,d,state.dashStatus));
 ...
 let q=state.search.trim().toLowerCase();if(q)rows=rows.filter(r=>[r.reqCode,r.subject,r.serviceName,r.requesterName,r.departmentSnapshot].some(v=>String(v||'').toLowerCase().includes(q)));
...
function loginView(){return `<div class="login-wrap"><div class="login-card"><h1>نظام تسجيل الجودة</h1><p class="muted">تسجيل الدخول بحساب الدليل (Active Directory) أو بحساب محلي</p><div id="loginErr"></div><form id="loginForm"><label>اسم المستخدم أو البريد الرسمي</label><input name="email" type="text" autocomplete="username" autocapitalize="off" spellcheck="false" required><label>كلمة المرور</label><input name="password" type="password" autocomplete="current-password" required><button class="primary full-btn">تسجيل الدخول</button></form><div class="demo-users"><strong>حسابات الاختبار المحلية</strong><p>مستخدم: hisham@test.local</p><p>مشرف قسم: supervisor@test.local</p><p>Power User: power@test.local</p><p>مدير النظام: admin@test.local</p><p>كلمة المرور للجميع: <b>Test123</b></p><p class="small muted">موظفو الإدارة يسجّلون الدخول باسم المستخدم في الدليل وكلمة مروره.</p></div></div></div>`}
function forcePasswordView(){return `<div class="login-wrap"><div class="login-card"><h1>تغيير كلمة المرور مطلوب</h1><p class="muted">تم تعيين كلمة مرور مؤقتة لهذا الحساب.</p><div id="forceErr"></div><form id="forcePasswordForm"><label>كلمة المرور الجديدة</label><input type="password" name="password" minlength="6" required><label>تأكيد كلمة المرور</label><input type="password" name="confirmPassword" minlength="6" required><button class="primary full-btn">حفظ كلمة المرور</button></form></div></div>`}
function passwordView(){return `<div class="page-title"><div><h1>تغيير كلمة المرور</h1><p>يمكن للمستخدم تغيير كلمة مروره بنفسه.</p></div></div><section class="form-card"><form id="passwordForm"><label>كلمة المرور الحالية *</label><input type="password" name="current" required><label>كلمة المرور الجديدة *</label><input type="password" name="password" minlength="6" required><label>تأكيد كلمة المرور الجديدة *</label><input type="password" name="confirmPassword" minlength="6" required><button class="primary top-gap">حفظ</button></form></section>`}
...
function dashboard(d,u){let rows=filteredRequests(d,u),m=cardMetrics(rows,d),showEmployee=u.role!=='user',showDepartment=u.role==='power',active=state.dashStatus;let card=(label,val,filter,cls='')=>`<div class="card clickable ${cls} ${active===filter?'selected-filter':''}" data-card-filter="${filter}"><span>${label}</span><b>${val}</b></div>`;return `<div class="page-title"><div><h1>لوحة التحكم</h1><p>متابعة الطلبات حسب صلاحيات المستخدم والفلاتر الحالية.</p></div><div class="page-actions"><button class="primary" data-nav="new">+ طلب جديد</button><button class="primary export-btn" id="exportRequests">تصدير إلى Excel</button></div></div>${dashboardFilterBar(d,u)}<div class="cards">${card('إجمالي الطلبات',rows.length,'all')}${card('المفتوحة',m.open.length,'Open')}${card('المغلقة',m.closed.length,'Closed')}${card('المتأخرة',m.over.length,'Overdue','warn')}${card('تستحق قريباً',m.soon.length,'DueSoon')}<div class="card"><span>الإنجاز في الوقت</span><b>${m.closed.length?m.pct+'%':'-'}</b></div></div>${u.role==='supervisor'||u.role==='power'?chartsView(rows,d):''}<section><div class="section-head"><h2>الطلبات</h2><div class="small muted">الجدول يعرض نفس نتائج الفلاتر أعلاه.</div></div><div class="table-wrap"><table id="reqTable"><thead><tr><th>رقم الطلب</th>${showEmployee?'<th>الموظف</th>':''}${showDepartment?'<th>القسم</th>':''}<th>الخدمة</th><th>الموضوع</th><th>تاريخ الطلب</th><th>الموعد النهائي</th><th>الحالة</th><th>أيام التأخير</th><th>الإجراء</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r.reqCode}</td>${showEmployee?`<td>${r.requesterName}${r.userId===u.id?' <span class="mini-own">طلبي</span>':''}</td>`:''}${showDepartment?`<td>${r.departmentSnapshot}</td>`:''}<td>${r.serviceName}</td><td>${r.subject}</td><td>${r.requestDate}</td><td>${r.dueDate}</td><td><span class="status ${r.status==='Closed'?'closed':'open'}">${r.status==='Closed'?'مغلق':'مفتوح'}</span>${r.status==='Open'&&r.dueDate<TODAY?'<span class="status late">متأخر</span>':''}</td><td>${calendarLate(r)?`<strong class="delay-days">${currentDelayDays(r,d)}</strong>`:'-'}</td><td><span class="link" data-open="${r.reqCode}">عرض</span></td></tr>`).join('')||`<tr><td colspan="${showDepartment?10:showEmployee?9:8}" class="empty">لا توجد طلبات مطابقة</td></tr>`}</tbody></table></div></section>${u.role==='power'?departmentPerformance(rows,d):''}`}
function newView(d,u){let sv=d.services.filter(s=>s.departmentId===u.departmentId&&s.active);return `<div class="page-title"><div><h1>تسجيل طلب جديد</h1><p>سيتم إنشاء رقم الطلب والموعد النهائي تلقائياً.</p></div></div><section class="form-card"><form id="newForm"><div class="grid2"><div><label>مقدم الطلب</label><input value="${u.name}" disabled></div><div><label>البريد الرسمي</label><input value="${u.email}" disabled></div><div><label>القسم المعني</label><input value="${depName(d,u.departmentId)}" disabled></div><div><label>نوع الخدمة *</label><select name="serviceId" required><option value="">اختر الخدمة</option>${sv.map(s=>`<option value="${s.id}">${s.code} - ${s.name} (${s.duration} يوم عمل)</option>`).join('')}</select></div><div><label>تاريخ تسجيل الطلب *</label><input type="date" name="requestDate" max="${TODAY}" value="${TODAY}" required></div><div><label>موضوع الطلب *</label><input name="subject" required></div></div><label>الملاحظات</label><textarea name="notes" rows="4"></textarea><button class="primary top-gap">تسجيل الطلب</button></form></section>`}
...
function render(){let d=load(),u=user(),root=document.getElementById('app');
 ...
 if(u.forcePasswordChange){root.innerHTML=forcePasswordView();bindForcePassword();return}let body=state.view==='dashboard'?dashboard(d,u):state.view==='new'?newView(d,u):state.view==='detail'?detailView(d,u,state.reqCode):state.view==='admin'?adminView(d,u):state.view==='password'?passwordView():dashboard(d,u);root.innerHTML=shell(body,d,u);bindCommon(d,u);state.flash=null}
function bindLogin(){
 ...
 form.onsubmit=async e=>{
  ...
  try{
   ...
   state.view='dashboard';state.flash=null;
   render();
  }catch(ex){
...
function bindCommon(d,u){document.querySelectorAll('[data-nav]').forEach(b=>b.onclick=()=>{state.view=b.dataset.nav;state.reqCode=null;render()});let rf=document.getElementById('refreshData');if(rf)rf.onclick=async()=>{
 ...
 let pf=document.getElementById('passwordForm');if(pf)pf.onsubmit=async e=>{
  e.preventDefault();
  const f=new FormData(pf);
  if(f.get('password')!==f.get('confirmPassword')){state.flash={type:'danger',text:'كلمتا المرور الجديدتان غير متطابقتين.'};render();return}
  // The current password is verified server-side against the stored hash. It
  // was compared here, in plain text, against a password the browser was
  // holding — which is why the browser no longer holds one.
  try{
   await API.changePassword(f.get('current'),f.get('password'),f.get('confirmPassword'));
   await refresh();
   state.flash={type:'success',text:'تم تغيير كلمة المرور بنجاح.'};state.view='dashboard';render();
  }catch(ex){state.flash={type:'danger',text:ex.message};render()}
 };
 let nf=document.getElementById('newForm');if(nf)nf.onsubmit=async e=>{
  e.preventDefault();
  ...
  if(created){state.view='detail';state.reqCode=created.request.reqCode;render()}
 };
...
function bindAdmin(d,u){document.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>{state.adminTab=b.dataset.tab;state.adminForm=null;state.adminSearch='';state.adminDept=state.adminRole=state.adminStatus='all';if(state.adminTab==='audit'){DATA.audits=[];render();loadAudit();return}render()});let as=document.getElementById('adminSearch');if(as)as.oninput=e=>{state.adminSearch=e.target.value;render()};let ad=document.getElementById('adminDept');if(ad)ad.onchange=e=>setAndRender('adminDept',e.target.value);let ar=document.getElementById('adminRole');if(ar)ar.onchange=e=>setAndRender('adminRole',e.target.value);let ast=document.getElementById('adminStatus');if(ast)ast.onchange=e=>setAndRender('adminStatus',e.target.value);let add=document.getElementById('addAdminItem');if(add)add.onclick=()=>{state.adminForm={mode:'add'};render()};document.querySelectorAll('[data-edit-admin]').forEach(b=>b.onclick=()=>{state.adminForm={mode:'edit',id:Number(b.dataset.editAdmin)};render()});let cancel=document.getElementById('cancelAdminForm');if(cancel)cancel.onclick=()=>{state.adminForm=null;render()};
 ...
 document.querySelectorAll('[data-reset-user]').forEach(b=>b.onclick=()=>{
  const x=(load().users||[]).find(z=>z.id===Number(b.dataset.resetUser));
  const pw=prompt(`أدخل كلمة مرور مؤقتة جديدة للمستخدم ${x?x.name:''}:`,'Temp123');
  if(!pw)return;
  if(pw.length<6){alert('كلمة المرور يجب أن تكون 6 أحرف على الأقل.');return}
  mutate(()=>API.resetUserPassword(Number(b.dataset.resetUser),pw),'تمت إعادة كلمة المرور وسيُطلب من المستخدم تغييرها عند الدخول.');
 });
